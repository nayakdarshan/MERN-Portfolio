...
function AdminIntro() {
  ...
  const onFinish = async (values) => {
    if (isGuest) {
      message.error('Guest users cannot update the introduction.');
      return;
    }
    ...
    <div>
      <Form layout='vertical' onFinish={onFinish} initialValues={portfolioData.intro}>
        <Form.Item name='welcomeText' label="Enter Welcome Text">
          <Input placeholder='Welcome Text' disabled={isGuest} />
        </Form.Item>

        <Form.Item name='caption' label="Enter Caption">
          <Input placeholder='Caption' disabled={isGuest} />
        </Form.Item>

        <Form.Item name='description' label="Enter Description">
          <Input.TextArea placeholder='Description' style={{ minHeight: '150px' }} disabled={isGuest} />
        </Form.Item>
